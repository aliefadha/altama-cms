...
const DEFAULT_GOOGLE_STORAGE_URL = 'http://localhost:3003'

export const API_BASE_URL = import.meta.env.VITE_API_BASE_URL
    ? `${import.meta.env.VITE_API_BASE_URL}`.replace(/\/$/, '')
    : DEFAULT_API_BASE_URL

export const GOOGLE_STORAGE_URL = import.meta.env.VITE_GOOGLE_STORAGE_URL
    ? `${import.meta.env.VITE_GOOGLE_STORAGE_URL}`.replace(/\/$/, '')
    : DEFAULT_GOOGLE_STORAGE_URL
...
export function getImageUrl(imagePath: string): string {
    ...
    return `${GOOGLE_STORAGE_URL}${imagePath.startsWith('/') ? '' : '/'}${imagePath}`
}
